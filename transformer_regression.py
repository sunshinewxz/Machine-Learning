...
# from daisy.models.transformer import TransformerModel
...
def point_wise_feed_forward_network(d_model, dff):
  return tf.keras.Sequential([
      tf.layers.Dense(dff, activation='relu'),  # (batch_size, seq_len, dff)
      tf.layers.Dense(d_model)  # (batch_size, seq_len, d_model)
  ])
...
class MultiHeadAttention(tf.layers.Layer):
    def __init__(self, d_model, num_heads):
        ...
        self.wq = tf.layers.Dense(d_model)
        self.wk = tf.layers.Dense(d_model)
        self.wv = tf.layers.Dense(d_model)
...
class Encoder(tf.layers.Layer):
    def __init__(self, num_layers, d_model, num_heads, dff, input_vocab_size, rate=0.1):
        ...
        self.embedding = tf.layers.Dense(self.d_model)
